...
## 1.3 Tenant Isolation Model — ⚠️ ยังไม่ตัดสินใจ (แต่ cross-reference แก้แล้ว)

`PLATFORM_COMPONENTS.md` ส่วน Data Layer อ้างอิงว่ารูปแบบ isolation ต่อ tenant "ยังไม่ตัดสินใจสุดท้าย —
ดู `ENTERPRISE_ARCHITECTURE.md` หัวข้อ 'Tenant Isolation Model'"

✅ **แก้ไขแล้ว:** เดิม cross-reference นี้ชี้ไปที่ "ข้อ 6" ซึ่งไม่มีอยู่จริง — ตอนนี้ `ENTERPRISE_ARCHITECTURE.md`
มีหัวข้อ **"Tenant Isolation Model"** แล้ว (อยู่ระหว่าง Data Platform Layer กับ Infrastructure Layer)
แต่**การตัดสินใจเรื่อง model ยังไม่เสร็จ** ยังเป็น Open Decision เหมือนเดิม เพียงแค่ตอนนี้มีที่ให้
อ้างอิงกลับไปจริงๆ แล้ว
...
การตัดสินใจสุดท้ายต้องบันทึกเป็น ADR ใหม่ (ADR-0010) ตามกฎ Architecture Decision Rule
...
- ~~`PLATFORM_COMPONENTS.md` อ้างอิง `ENTERPRISE_ARCHITECTURE.md ข้อ 6` เรื่อง tenant isolation~~
  **แก้แล้ว (2026-08-02)** — ดูหัวข้อ 1.3 ด้านบน
