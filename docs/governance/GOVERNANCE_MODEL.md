...
> เชื่อมโยงกับ `docs/adr/` (ยังว่างอยู่), `governance_metrics` ใน `CANONICAL_DATA_MODEL.md`,
> และรายการที่ยังไม่ตัดสินใจซึ่งสะสมมาจาก Sprint 1-3
...
- สถานะปัจจุบัน: `docs/adr/` ยังไม่มีไฟล์เลย ทั้งที่มี 6 ประเด็นค้างอยู่แล้ว (ดูข้อ 3.1)
  — Sprint 4 ควรเริ่มสร้างไฟล์ ADR อย่างน้อยสำหรับประเด็นที่กระทบงานเร่งด่วน (แอป 2) ก่อน
- ADR ที่เกี่ยวกับ security (เช่น รูปแบบ auth ระหว่างแอป) ต้อง cross-reference กับ
  `docs/security/SECURITY_MODEL.md`

### 3.1 คิว ADR ที่ต้องเปิด (ดึงมาจากสรุปโปรเจกต์ก่อนหน้า)

| # | ประเด็น | ความเร่งด่วน | เหตุผล |
|---|---|---|---|
| ADR-001 | Firestore แยก instance ต่อ tenant หรือ instance เดียวแบ่ง collection/prefix | สูง | กระทบ isolation model ทั้งแพลตฟอร์ม |
| ADR-002 | App 3 ต่อ Firestore จริงตั้งแต่ต้น หรือ mock-only ก่อน | กลาง | กระทบ timeline การพัฒนา |
| ADR-003 | API ระหว่าง App 3 กับ App 1/2: REST หรือ Firestore listener | กลาง | กระทบ `API_CONTRACT.md` |
| ADR-004 | `governance_metrics` เริ่มเก็บจริงเมื่อไหร่ | สูง | ผูกกับการแก้ security bug แอป 2 — ควรรอจนกว่าข้อ 4.1 (ล่าง) จะแก้เสร็จ |
| ADR-005 | ชื่อสุดท้ายของ App 3 | ต่ำ | ไม่กระทบ technical แต่กระทบเอกสาร/การสื่อสารลูกค้า |
| ADR-006 | ขอบเขตสิทธิ์ลูกค้าเห็นได้ใน Live mode (สถานะอย่างเดียว หรือเห็นข้อมูลดิบ) | สูง | กระทบทั้ง security model และ contract กับลูกค้า |
...
- ก่อนเปิด `governance_metrics` ให้เก็บข้อมูลจริง (ADR-004) ต้องแก้ security bug ในแอป 2
...
| Governance metrics เริ่มเก็บจริง | 🔴 ยังไม่เริ่ม — รอ ADR-004 และรอแก้ security bug ก่อน |

## 10. Open Items

- [ ] เปิดไฟล์ ADR-001 ถึง ADR-006 ตามคิวในข้อ 3.1
...
- [ ] ตัดสินใจว่า `governance_metrics` เริ่มเก็บจริงพร้อมกับการแก้ security bug แอป 2 เลย
      หรือรอให้แก้เสร็จสมบูรณ์ก่อน (เชื่อม ADR-004)